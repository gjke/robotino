...
ofstream myfile;
int lastDeflection = 0;
int maxDeflection = 160;
...
void getOI()
{
    ...
    try
	{
		...
		//imwrite("C:\\Users\\1130386.FFE\\opencvtest\\ihsv.jpg", hsv_src);
	...
	if (contours.size() > 0)
	{
		...
		/*if ((abs(oi.deflection) > maxDeflection) || (abs(oi.deflection - lastDeflection) > 30))
		{
			oi.deflection = lastDeflection;	
		}
		else
		{
			lastDeflection = oi.deflection;
		}*/
...
rec::core::image::Image removeObject(vector<Point2i>* contour, int area)
{
	Rect boundRect = boundingRect(Mat(*contour));
	int shift = (int) area / 3;
	boundRect = boundRect - Point2i(5, 5);
